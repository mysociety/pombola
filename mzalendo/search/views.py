...
def tagcloud(request,wks=4):
    ...
    cutoff = SearchQuerySet().models(hansard_models.Entry).latest('sitting__start_date')
    sqs  = SearchQuerySet().models(hansard_models.Entry).filter(sitting__pk__gte=(int(cutoff.pk)-int(wks)))
    cloudlist =[]
    try:
