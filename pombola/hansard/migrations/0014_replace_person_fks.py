...
from pombola.core.migration_helpers import get_new_object_id

class Migration(DataMigration):

    def forwards(self, orm):
        qs = orm.Entry.objects.filter(speaker__isnull=False)
        total_entries = qs.count()
        entries_done = 0
        for entry in qs:
            entry.new_speaker_id = get_new_object_id(
                orm,
                entry.speaker_id,
                ('core', 'person')
                )
            entry.save()
            entries_done += 1
            if (entries_done % 500) == 0:
                print "Migrated {done} out of {total} Hansard entries ({pc:.0f}%)".format(
                    done=entries_done,
                    total=total_entries,
                    pc=(100.0*entries_done/total_entries)
                )
        for alias in orm.Alias.objects.filter(person__isnull=False):
            alias.new_person_id = get_new_object_id(
                orm,
                alias.person_id,
                ('core', 'person')
                )
    ...
    models = {
        ...
        'hansard.alias': {
            ...
            'new_person': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['core.PopoloPerson']", 'null': 'True', 'blank': 'True'}),
        ...
        'hansard.entry': {
            ...
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'new_speaker': ('django.db.models.fields.related.ForeignKey', [], {'blank': 'True', 'related_name': "'hansard_entries'", 'null': 'True', 'to': u"orm['core.PopoloPerson']"}),
            ...
            'speaker': ('django.db.models.fields.related.ForeignKey', [], {'blank': 'True', 'related_name': "'old_hansard_entries'", 'null': 'True', 'to': u"orm['core.Person']"}),
    ...
    complete_apps = ['popolo', 'core', 'hansard']
    symmetrical = True
