...
def parse_approximate_date(s):
    ...
    for regexp in [r"^(\d{4})-(\d{2})-(\d{2})$", r"^(\d{4})-(\d{2})$", r"^(\d{4})$"]:
...
class Command(LabelCommand):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.organisation_executive = Organisation.objects.get(
    ...
    def import_executive(self, person_csv):
        ...
        # Find the person
        person = self.match_person(
            person_csv["First names"].strip() + " " + person_csv["Surname"].strip()
        )
    ...
    def import_mpls(self, person_csv):
        ...
        # Find the person
        person = self.match_person(
            person_csv["First Name"].strip() + " " + person_csv["Last Name"].strip()
        )

        # This person might have phone numbers; if so set it
        if person_csv["Cell phone"]:
            self.save_contact_list(
                person_csv["Cell phone"], person, self.contact_kind_phone
            )
        ...
        organisation = self.match_organisation(
            person_csv["Organisation"], "provincial-legislature"
        )
    ...
    def import_iec_assignment(self, person_csv):
        ...
        # Find the person
        person = self.match_person(
            person_csv["Name"].strip() + " " + person_csv["Surname"].strip()
        )
