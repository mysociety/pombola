...
class Migration(DataMigration):
    def forwards(self, orm):
        for place in orm.Place.objects.filter(organisation__isnull=False):
            place.organization_id = get_new_object_id(
                orm,
                place.organisation_id,
                ('core', 'organisation')
                )
            place.save()

        for ps in orm.ParliamentarySession.objects.all():
            ps.new_house_id = get_new_object_id(
                orm,
                ps.house_id,
                ('core', 'organisation')
                )
        ...
        for relationship in orm.OrganisationRelationship.objects.all():
            relationship.organization_a_id = get_new_object_id(
                orm,
                relationship.organisation_a_id,
                ('core', 'organisation')
                )
            relationship.organization_b_id = get_new_object_id(
                orm,
                relationship.organisation_b_id,
                ('core', 'organisation')
                )
            relationship.save()
