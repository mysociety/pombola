...
class Migration(DataMigration):
    ...
    depends_on = (
        ('core', '0060_drop_slug_redirect'),
    )

    def trim(self, s, max_length):
        if s is None:
            return s
        return s[:max_length]

    def forwards(self, orm):
        '''Truncate name and summary from PopIt data

        This is to enable the SayIt migration to django-popolo'''

        for p in orm['popit.Person'].objects.all():
            p.name = self.trim(p.name, 128)
            p.summary = self.trim(p.summary, 512)
            p.save()

        for s in orm['speeches.speaker'].objects.all():
            s.name = self.trim(s.name, 128)
            s.save()
