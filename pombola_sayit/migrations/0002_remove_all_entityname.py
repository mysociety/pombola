...
class Migration(DataMigration):
    ...
    needed_by = (
        ('speeches', '0051_move_speaker_data'),
    )

    def forwards(self, orm):
        """Remove all EntityName objects, which are transient anyway

        If we don't remove these before the SayIt migration to
        django-popolo there'll be a foreign key constraint violation
        exception when the Person objects from popit_django are
        removed, because they're referred to by EntityName objects.
        """

        for en in orm['popit_resolver.EntityName'].objects.all():
            en.delete()
    ...
    models = {
        ...
        u'core.identifier': {
            ...
            'content_type': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'pombola_identifier_set'", 'to': u"orm['contenttypes.ContentType']"}),
            'created': ('django.db.models.fields.DateTimeField', [], {'auto_now_add': 'True', 'blank': 'True'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
        ...
        u'mapit.generation': {
            'Meta': {'ordering': "('id',)", 'object_name': 'Generation'},
    ...
    complete_apps = ['popit_resolver', 'core', 'pombola_sayit']
    symmetrical = True
